...
Blockly.Blocks['output_cout'] = {
  init: function() {
	...
	decompose: function(workspace){
		var containerBlock = Blockly.Block.obtain(workspace, 'cout_stream_mutator');
		containerBlock.initSvg();

		var connection = containerBlock.getInput('STACK').connection;
		
		for(var i = 1; i <= this.coutStreamCount_; i++){
			var printAddBlock = Blockly.Block.obtain(workspace, 'cout_stream_add');
	...
	compose: function(containerBlock){
		for(var i = 1; this.coutStreamCount_; i++){
		...
		while(clauseBlock){
			switch(clauseBlock.type){
				case 'cout_stream_add':
					...
					var printInput = this.appendValueInput('valinp' + this.cout_stream_add)
		...
		while(clauseBlock){
			switch(clauseBlock.type){
				case 'cout_stream_add':
					...
					clauseBlock.valueConnection = inputPrint && inputPrint.connection.targetConnection;
